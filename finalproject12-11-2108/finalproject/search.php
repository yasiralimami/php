				...
				<section class="col-lg-8">
					...
					<?php
						if(isset($_GET['search_submit'])){
							/* show box panel with search input value */
							echo '
							...
							';
							// put the seach statement
							$stmt = $conn->prepare("SELECT * FROM posts WHERE title LIKE '%$_GET[search]%' OR description LIKE '%$_GET[search]%'");
						$stmt->execute();
						 while ($rows = $stmt->fetch(PDO::FETCH_ASSOC))
	                       { //show the posts that contian the search keywords
							 echo '
